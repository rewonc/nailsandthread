...
var Grid = {
  ...
  storePixels: function (grid, pixels) {
    grid.pixelStore = [];
    if (pixels.data.length % grid.size !== 0) console.log(
      "storePixels ratio uneven; will affect results");
    ...
    var aggregateColors = function (element, ratio, pixels) {
      ...
      var addToRes = function (count) {
        var cmyk = Grid.helpers.RGBtoCMYK({
          red: pixels.data[count],
          green: pixels.data[count + 1],
          blue: pixels.data[count + 2]
        });
        res.c += cmyk.c;
        res.m += cmyk.m;
        res.y += cmyk.y;
  ...
  draw: function (grid, origin, next, pixelLine, pixels, thread,
    pixelsToRender) {
  ...
  findNextByWalking: function (origin, grid, pixels, thread) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (Grid.helpers.checkGridValidity(origin, list[i], grid) === true) {
        ...
        if (Grid.helpers.checkValidity(pixelLine, thread)) return {
          pixelLine: pixelLine,
          next: list[i],
          prev: origin
        };
  ...
  helpers: {
    farthestNodesFrom: function (origin, grid) {
      ...
      return _.sortBy(arr, function (val) {
        ...
        var columnSquared = Math.pow((val - row2 * grid.height) - (origin -
          row1 * grid.height), 2);
    ...
    nodesAdjacentTo: function (origin, grid, radius, mock) {
      ...
      for (var i = rcPoint.row - radius; i <= rcPoint.row + radius; i++) {
        for (var j = rcPoint.column - radius; j <= rcPoint.column + radius; j++) {
          if (i < 0 || j < 0 || i >= grid.height || j >= grid.width || (i ===
            rcPoint.row && j === rcPoint.column)) {
            //do nothing
          } else {
    ...
    getPixels: function (origin, next, grid, pixels) {
      ...
      if (pixelLength % gridLength !== 0) console.log(
        'Warning: Grid/pixel ratio off balance. Balance for better results.'
      );
      ...
      var originImg = Grid.helpers.scaleToImage(rcOrigin, grid.width, grid.height,
        pixels.width, pixels.height);
      var nextImg = Grid.helpers.scaleToImage(rcNext, grid.width, grid.height,
        pixels.width, pixels.height);
    ...
    pixellate: function (origin, next, slope, pixels) {
      ...
      if (slope.start_with === "rows") {
        for (; i < slope.count; i++) {
          (function (i) {
            var index = Grid.helpers.rcToPixels(origin.row + (i * slope.increment),
              Math.round(origin.column + i * slope.slope), pixels.width,
              pixels.height, 0);
            res.push({
              index: index,
        ...
        for (; i < slope.count; i++) {
          (function (i) {
            var index = Grid.helpers.rcToPixels(Math.round(origin.row + i *
                slope.slope), origin.column + (i * slope.increment),
    ...
    scaleToImage: function (rcPoint, width1, height1, width2, height2) {
      return {
        row: height2 / height1 * rcPoint.row,
        column: width2 / width1 * rcPoint.column
      };
    },
